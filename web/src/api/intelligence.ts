...
/** Reverse lineage: outputs generated by a run that pinned this entity
 *  (e.g. every image produced with a given checkpoint). */
export async function getConsumers(
  code: string,
): Promise<{ entity: string; consumer_count: number; consumers: EntityConsumer[] }> {
  const { data } = await http.get(`${BASE}entities/${code}/consumers/`);
  return data;
}
