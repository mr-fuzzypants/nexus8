...
- **F4 — the curated dependency graph remains too convoluted for artist use; H4 refuted (August 1 2026).** Even after semantic aggregation (iteration 2) and query-plan optimization (iteration 3), the resulting graph is not legible to an artist in practice. The structure of the relationship stores — heterogeneous granularities, split layer identity, per-variation lineage fan-out — produces a graph topology that is difficult to read and navigate regardless of visual encoding. **Status: fail.** The graph view is retained as a developer/debugging surface but should not be presented to artists as a navigational tool.
